...
NTSTATUS getProcessImageName(HANDLE hProcess, PUNICODE_STRING* ppProcessImageName)
{
	...
	__try
	{
		...
		pBuffer = ExAllocatePool2(POOL_FLAG_NON_PAGED, nBufferSize, c_nAllocTag);
...
NTSTATUS getProcessList(HANDLE** ppProcessArray, size_t* pnSize)
{
	...
	__try
	{
		...
		pSystemProcessInfo = (PSYSTEM_PROCESS_INFORMATION)ExAllocatePool2(
			POOL_FLAG_NON_PAGED, nBufferSize, c_nAllocTag);
		...
		pProcessArray = (HANDLE*)ExAllocatePool2(POOL_FLAG_NON_PAGED,
