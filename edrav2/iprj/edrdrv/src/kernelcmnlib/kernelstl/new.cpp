...
//void* __cdecl operator new (size_t nSize, POOL_FLAGS ePoolType)
//{
//	nSize = (nSize != 0) ? nSize : 1;
//
//	PVOID result = ExAllocatePool2(ePoolType, nSize, cmd::c_nAllocTag);
//	if (result)
...
{
	nSize = (nSize != 0) ? nSize : 1;
	PVOID result = ExAllocatePool2(convetrToSystemPoolType(ePoolType), nSize, cmd::c_nAllocTag);
	if (result)
		RtlZeroMemory(result, nSize);
	return result;
}
