...
NTSTATUS getTokenSid(HANDLE hToken, PUNICODE_STRING pusUserName)
{
	...
	__try
	{
		...
		pTokenUser = (TOKEN_USER*)ExAllocatePool2(POOL_FLAG_NON_PAGED, nTokenInfoSize, c_nAllocTag);
...
SECURITY_DESCRIPTOR* createFltPortEveryoneAllAccessAllowMandantorySD()
{
	...
	UniquePtr<SECURITY_DESCRIPTOR> securityDescriptor(reinterpret_cast<SECURITY_DESCRIPTOR*>(new (PoolType::NonPaged) UINT8[securityDescriptorAllocSize]));
	...
	)
{
	...
	cmd::UserSpaceRegionPtr regionPtr(new (PoolType::NonPaged) cmd::UserSpaceRegion());
