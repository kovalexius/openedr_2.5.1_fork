...
NTSTATUS setDeviceDacl(PDEVICE_OBJECT DeviceObject, DWORD secLevel)
{
	...
	for (;;)
	{
		...
		pAcl = (PACL)ExAllocatePool2(POOL_FLAG_PAGED, aclSize, MEM_TAG);
		if (pAcl == NULL) 
		{
...
#pragma warning(disable: 28197) 
					pPacket->flatStreamData = 
						(char*)ExAllocatePool2(POOL_FLAG_NON_PAGED, pPacket->streamData.dataLength, MEM_TAG_TCP_DATA);
#pragma warning(pop)
...
ULONG devctrl_processTcpPacket(PNF_DATA pData)
{
	...
	for (;;) 
	{
		...
		if (pData->bufferSize > 0)
		{
			dataCopy = ExAllocatePool2(POOL_FLAG_NON_PAGED, pData->bufferSize, MEM_TAG_TCP_DATA_COPY);
			if (dataCopy == NULL)
			{
				status = STATUS_NO_MEMORY;
...
#pragma warning(disable: 28197) 
			pPacket->controlData = (WSACMSGHDR*)
				ExAllocatePool2(POOL_FLAG_NON_PAGED, pPacket->options.controlDataLength, MEM_TAG_UDP_DATA);
#pragma warning(pop)
		...
		dataCopy = ExAllocatePool2(POOL_FLAG_NON_PAGED, dataLength, MEM_TAG_UDP_DATA_COPY);
		if (dataCopy == NULL)
		{
			status = STATUS_NO_MEMORY;
...
ULONG devctrl_processIPPacket(PNF_DATA pData)
{
	...
	for (;;) 
	{
		...
		dataCopy = ExAllocatePool2(POOL_FLAG_NON_PAGED, dataLength, MEM_TAG_IP_DATA_COPY);
		if (dataCopy == NULL)
		{
			status = STATUS_NO_MEMORY;
